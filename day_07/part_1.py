...
def get_total_size_sum(input_file: str) -> int:
    with open(input_file, 'r') as f:
        lines = f.read().splitlines()
    parser = TreeParser()
    filetree = parser.parse(lines)
    relevant_sizes = [size for dir, size in filetree.dir_sizes().items() if size <= MAX_SIZE]
    return sum(relevant_sizes)


def main():
    assert get_total_size_sum("test_input.txt") == 95437
    answer = get_total_size_sum("input.txt")
    print(f"The sum of total sizes below the threshold is: {answer}")
